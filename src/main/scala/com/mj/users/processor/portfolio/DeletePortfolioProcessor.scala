...
import com.mj.users.mongo.PortfolioDao.portfolioCollection
...
class DeletePortfolioProcessor extends Actor with MessageConfig{
  ...
  def receive = {
    ...
    case (memberID: String , expID : String) => {
      ...
      val result = remove(portfolioCollection, document("memberID" -> memberID , "mediaID" -> expID))
