...
import com.mj.users.mongo.PublicationDao.publicationCollection
import reactivemongo.bson.document

import scala.concurrent.ExecutionContext.Implicits.global
import scala.concurrent.Future

class DeletePublicationProcessor extends Actor with MessageConfig{
  ...
  def receive = {
    ...
    case (memberID: String , pubID : String) => {
      ...
      val result = remove(publicationCollection, document("memberID" -> memberID , "pubID" -> pubID))
