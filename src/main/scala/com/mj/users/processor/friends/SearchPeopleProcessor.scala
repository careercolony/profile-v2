    ...
    case memberID : String => {
      ...
      val records = MutableList[User]()
        while (response.hasNext()) {
          ...
          val user: User = new User(record.get("b.memberID").asInt, record.get("b.firstname").asString(), record.get("b.lastname").asString(), record.get("b.email").asString(),record.get("b.avatar").asString(), record.get("degree").asInt())
          records += user
        }
